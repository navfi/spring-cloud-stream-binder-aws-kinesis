...
Default: `false`
region::
    Effective only if `kplKclEnabled` is set to `true`.
Which region to send records to. If you do not specify the region and are running in EC2, the library will use the region the instance is in.
+
Default: `US_EAST_1` for consumers. For producer, if are running in EC2, the library will use the region the instance is in.
